    ...
    def deconstruct_claims(self, answer: str) -> List[str]:
        ...
        for sentence in sentences:
            ...
            if any(skip in sentence.lower() for skip in [
                'this answer', 'this response', 'the above', 'note:', 'in summary',
                'question:', 'answer:', 'source:', 'evidence', 'part '
            ]):
                continue
            
            # Prioritize claims with quantitative data (numbers, percentages, p-values, etc.)
            has_quantitative = bool(re.search(r'\d+[.%]|\bp\s*[<>=]\s*\d|confidence|interval|sample\s*size', sentence, re.IGNORECASE))
            
            # Include claims with specific findings, outcomes, or results
            has_finding = any(term in sentence.lower() for term in [
                'showed', 'found', 'demonstrated', 'indicated', 'revealed',
                'result', 'outcome', 'efficacy', 'safety', 'response',
                'rate', 'percentage', 'improvement', 'reduction', 'increase'
            ])
...
Answer (only 'Supports', 'Refutes', or 'Not Mentioned'):"""
            ...
            try:
                ...
                elif 'REFUTES' in response or response.startswith('REFUTE'):
                ...
                logger.warning(f"Verification check failed for claim '{claim[:50]}...': {e}")
                continue
    ...
    def verify_answer(self, answer: str, source_chunks: List[str]) -> List[Dict[str, any]]:
        """
        ...
        logger.info(f"Verifying {len(claims)} claims against {len(source_chunks)} source chunks")
    ...
    def annotate_answer(self, answer: str, verification_results: List[Dict[str, any]]) -> str:
        """
