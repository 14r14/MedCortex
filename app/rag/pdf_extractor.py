...
def _extract_by_font_analysis(fileobj: BinaryIO) -> str | None:
    ...
    try:
        ...
        with pdfplumber.open(fileobj) as pdf:
            ...
            for char in title_chars:
                ...
                if current_y is None or abs(char_y - current_y) < 5:
                    ...
                    if current_line:
                        line_text = "".join(
                            c["text"]
                            for c in sorted(current_line, key=lambda c: c["x0"])
                        )
                        if line_text.strip():
            ...
            if current_line:
                line_text = "".join(
                    c["text"] for c in sorted(current_line, key=lambda c: c["x0"])
                )
                if line_text.strip():
                    title_lines.append(line_text.strip())
...
def _extract_by_position_pypdf(fileobj: BinaryIO) -> str | None:
            ...
            if best_idx + 1 < len(lines):
                ...
                if not _is_non_title_pattern(next_line):
                    ...
                    if 10 <= len(next_line) <= 150:
                        ...
                        if (
                            "by " not in next_line.lower()
                            and "author:" not in next_line.lower()
                        ):
...
def _extract_author_by_font(
    fileobj: BinaryIO, title_y: float | None = None
) -> str | None:
    ...
    try:
        ...
        with pdfplumber.open(fileobj) as pdf:
            ...
            candidate_font_sizes = (
                sorted_font_sizes[1:3]
                if len(sorted_font_sizes) > 1
                else sorted_font_sizes[:1]
            )
...
def _extract_author_by_position(
    fileobj: BinaryIO, title_line_idx: int | None = None
) -> str | None:
    ...
    try:
        ...
        for i in range(start_idx, end_idx):
            ...
            if any(
                ...
                for pattern in ["abstract", "keywords", "introduction", "1.", "2."]
            ):
                break
...
def _extract_author_improved(
    fileobj: BinaryIO,
    title_position: float | None = None,
    ...
    try:
        ...
        if candidates:
            ...
            logger.info(
                f"Selected author using {candidates[0][2]} method: {author[:50]}..."
            )
...
def extract_metadata(fileobj: BinaryIO) -> dict[str, str | None]:
    """
    ...
    try:
        ...
        try:
            ...
            if metadata:
                ...
                if metadata_title:
                    ...
                    if (
                        _is_non_title_pattern(metadata_title)
                        or len(metadata_title) < 10
                    ):
                        metadata_title = None
        except Exception as e:
        ...
        if candidates:
            ...
            logger.info(
                f"Selected title using {candidates[0][2]} method: {title[:50]}..."
            )
