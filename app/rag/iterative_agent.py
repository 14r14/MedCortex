...
Sub-questions:"""
    ...
    def retrieve_for_query(
        self, sub_query: str, allowed_doc_ids: list[str] | None = None
    ) -> list[dict]:
        ...
        keyword_hits = self.bm25.search(
            sub_query, top_k=15, allowed_doc_ids=allowed_doc_ids
        )
    ...
    def generate_followup_query(
        self, original_query: str, sub_queries: list[str], previous_results: list[dict]
    ) -> str | None:
        """
...
Follow-up question:"""
        ...
        try:
            # Use generate_from_prompt for custom prompt formatting
            followup = self.gen.generate_from_prompt(prompt=prompt, temperature=0.2)
...
Comprehensive answer:"""
    ...
    def answer_iteratively(
        ...
        if tables and self.table_reasoner.is_table_query(query, tables):
            logger.info(
                "Query appears to be about tabular data, performing table reasoning"
            )
    ...
    def _reciprocal_rank_fusion(
        self, semantic_hits: list[dict], keyword_hits: list[dict], k: int = 60
    ) -> list[dict]:
