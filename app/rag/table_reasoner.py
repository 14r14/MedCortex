...
class TableReasoner:
    ...
    def is_table_query(self, query: str, tables: list[dict[str, Any]]) -> bool:
        ...
        has_numerical = any(
            query_lower.startswith(op) or op in query_lower for op in numerical_ops
        )

        return has_keywords or has_numerical or len(tables) > 0

    def reason_over_tables(
        self, query: str, tables: list[dict[str, Any]], max_tables: int = 3
    ) -> str | None:
        """
...
Answer:"""
    ...
    def execute_table_query(self, query: str, table: dict[str, Any]) -> str | None:
        """
